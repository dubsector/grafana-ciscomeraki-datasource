...
func (d *DS) CallResource(ctx context.Context, req *backend.CallResourceRequest, send backend.CallResourceResponseSender) error {
	...
	default:
		return send(&backend.CallResourceResponse{Status: http.StatusNotFound})
	}
	if err != nil {
		return send(&backend.CallResourceResponse{Status: http.StatusInternalServerError, Body: []byte(err.Error())})
	}
	b, _ := json.Marshal(rows)
	return send(&backend.CallResourceResponse{
